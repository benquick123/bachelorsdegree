...
from old.twitter.Tweet import Tweet
...
def load_tweets(db_twitter, currency, date_from, date_to, p=False, _tweets=None):
    ...
    for tweet in (db_twitter.messages.find({"$and": [{"posted_time": {"$gte": date_from}}, {"posted_time": {"$lte": date_to}}, {"relevant": True}, {"crypto_currency": currency}]}) if _tweets is None else _tweets):
...
def calc_sentiment(db_twitter):
    sentiment_analyzer = SentimentIntensityAnalyzer()
    for tweet in db_twitter.messages.find({"in_event": {"$exists": True}}):
        scores_text = sentiment_analyzer.polarity_scores(tweet["striped_text"])
        db_twitter.messages.update_one({"_id": tweet["_id"]}, {"$set": {"sentiment_text": scores_text}})
