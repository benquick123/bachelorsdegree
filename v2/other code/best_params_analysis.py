...
def blankspace_fix(filename):
    f = open("results/parameter_search/conversations/" + filename, "r")
    s = f.read()
    f.close()
    i = s.find("]") + 1
    s = s[:i] + ", " + s[i:]
    f = open("results/parameter_search/conversations/" + filename, "w")
    f.write(s)
    f.close()
...
def parse_files():
    params = []
    for filename in os.listdir("results/parameter_search/conversations/"):
