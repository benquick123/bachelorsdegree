...
import old.news.functions as f_news
import old.twitter.functions as f_twitter
...
def create_events(currencies, date_from, date_to, db, plot=False):
    ...
    for currency in currencies:
        ...
        for i in range(len(moving_average)):
            ...
            if in_wait and ((prev_event_type == "up" and moving_average[i] > price_values[i]) or (prev_event_type == "down" and moving_average[i] < price_values[i])):
                in_wait = False
        ...
        for event in events_min_max:
            ...
            i = end-1
            while True:
                if ((event_type == "up" and price_values[i] <= event_price-moving_std[i]) or (event_type == "down" and price_values[i] >= event_price+moving_std[i])) and last_relevant_price is None:
                ...
                if (event_type == "up" and moving_average[i] > price_values[i]) or (event_type == "down" and moving_average[i] < price_values[i]):
                    if i != end:
                        ...
                        _event.time_x1 = i
                        _event.time_x2 = i if last_relevant_price is None else last_relevant_price["time_x"]
                        _event.time_x3 = end
...
def find_event_texts(db_events, db_twitter, db_news, db_trollbox, events, sorted_keys):
    # last_curr = ""
    # tweets = []
    ...
    for key in sorted_keys:
        ...
        date_end = datetime.utcfromtimestamp(event.last_relevant_price_info["_id"])
        ...
        #     last_curr = currency
        #     tweets = []
        #     for tweet in db_twitter.messages.find({"crypto_currency": currency.upper(), "$or": [{"relevant_predict": True}, {"relevant_classifier": True}]}):
        #         tweets.append(tweet)
...
def __main__():
    ...
    events = find_event_texts(db_events, db_twitter, db_news, db_trollbox, events, sorted_keys)

    # call this when different events are loaded. just once.
    # other_functions.mark_in_event(db_news, db_twitter, events)

    f_twitter.calc_sentiment(db_twitter)
